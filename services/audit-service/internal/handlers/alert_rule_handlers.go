...
type alertRuleService interface {
	...
	GetAlertInstances(ctx context.Context, filters models.AlertInstanceFilters) ([]models.AlertInstance, int, error)
	AcknowledgeAlert(ctx context.Context, instanceID, userID uuid.UUID, notes string, tenantID *uuid.UUID) error
	ResolveAlert(ctx context.Context, instanceID uuid.UUID, notes string, tenantID *uuid.UUID) error
}
...
// GetAlertInstances handles GET /api/v1/audit-service/alert-instances
func (h *AlertRuleHandler) GetAlertInstances(c *gin.Context) {
	filters := models.AlertInstanceFilters{
		Page:     1,
		PageSize: 50,
	}
	...
	if userType == middleware.UserTypeTenant {
		// Tenant users see only their tenant's alert instances
		tenantID := middleware.GetTenantID(c)
		if tenantID == nil {
			c.JSON(http.StatusForbidden, gin.H{"error": "Tenant context required"})
			return
	...
	// Platform users see all instances (no tenant filter)
	...
	instances, total, err := h.service.GetAlertInstances(c.Request.Context(), filters)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to retrieve alert instances"})
		return
	}
...
// AcknowledgeAlert handles POST /api/v1/audit-service/alert-instances/:id/acknowledge
func (h *AlertRuleHandler) AcknowledgeAlert(c *gin.Context) {
	idStr := c.Param("id")
	id, err := uuid.Parse(idStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid alert instance ID"})
		return
	}
	...
	err = h.service.AcknowledgeAlert(c.Request.Context(), id, userID, request.Notes, scope)
	if err != nil {
...
// ResolveAlert handles POST /api/v1/audit-service/alert-instances/:id/resolve
func (h *AlertRuleHandler) ResolveAlert(c *gin.Context) {
	idStr := c.Param("id")
	id, err := uuid.Parse(idStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid alert instance ID"})
	...
	err = h.service.ResolveAlert(c.Request.Context(), id, request.Notes, scope)
	if err != nil {
