...
func (hs *HealthScorer) CalculateHealthScore(metrics models.HealthMetrics) models.HealthScoreResponse {
	...
	resourceScore := hs.calculateResourceEfficiency(metrics)
	performanceScore := hs.calculatePerformanceMetrics(metrics)
	securityScore := hs.calculateSecurityPosture(metrics)
	businessScore := hs.calculateBusinessActivity(metrics)
	costScore := hs.calculateCostOptimization(metrics)

	// Calculate weighted overall score
	overallScore := (resourceScore * hs.weights.ResourceEfficiency) +
		(performanceScore * hs.weights.PerformanceMetrics) +
		(securityScore * hs.weights.SecurityPosture) +
		(businessScore * hs.weights.BusinessActivity) +
		(costScore * hs.weights.CostOptimization)

	// Determine health status
	healthStatus := hs.determineHealthStatus(overallScore)

	// Generate recommendations
	recommendations := hs.generateRecommendations(metrics, models.HealthBreakdown{
...
func (hs *HealthScorer) generateRecommendations(metrics models.HealthMetrics, breakdown models.HealthBreakdown) []models.Recommendation {
	...
	if breakdown.ResourceEfficiency < 70 {
	...
	if breakdown.PerformanceMetrics < 70 {
	...
	if breakdown.SecurityPosture < 70 {
	...
	if breakdown.BusinessActivity < 60 {
	...
	if breakdown.CostOptimization < 70 {
