 ...
 * twelve CVEs.
 *
 * `lens` is not decoration: `subject_type`/`subject_id`/`producer` are read
 * only by findings-scoped lenses. Keep the URL explicit so this link cannot
 * drift onto the crypto-risk stream if the page default changes.
