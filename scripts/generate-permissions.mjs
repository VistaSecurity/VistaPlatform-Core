...
//   services/auth-service/internal/rbac/role_delegations_gen.go (whole file)
//
...
function load() {
  ...
  // Role-delegation exceptions. See the `role_delegations:` block comment in
  // the YAML for the model; the checks here are what keep it honest.
  const delegations = reg.role_delegations || [];
  for (const d of delegations) {
    for (const k of Object.keys(d)) {
      if (!['grantor', 'grantee', 'permissions', 'note'].includes(k)) {
        fail(`role_delegations: unknown field: ${k}`);
      }
    }
    const grantor = roleByName.get(d.grantor);
    const grantee = roleByName.get(d.grantee);
    if (!grantor) fail(`role_delegations: unknown grantor role: ${d.grantor}`);
    if (!grantee) fail(`role_delegations: unknown grantee role: ${d.grantee}`);
    if (d.grantor === d.grantee) fail(`role_delegations: ${d.grantor} cannot delegate to itself`);
    if (!d.permissions?.length) fail(`role_delegations: ${d.grantor}->${d.grantee}: permissions required`);
    for (const n of d.permissions) {
      if (!byName.has(n)) fail(`role_delegations: unknown permission name: ${n}`);
    }
    // `permissions` must be EXACTLY the gap between the two roles' grant
    // filters: everything the grantee holds that the grantor does not.
    //
    // Under-listing leaves the exception dead — the guard still refuses the
    // role over the permission left out, which is the bug this mechanism
    // exists to fix, silently reintroduced. Over-listing exempts a permission
    // that is not actually in play, which is the "broadened predicate that
    // happens to admit the right answer today" shape a NAMED exception is
    // supposed to prevent. Pinning it to the computed gap also means a later
    // edit to either grant filter fails generation rather than quietly
    // widening or emptying the carve-out.
    const gap = permissions.filter((p) => grants(grantee, p) && !grants(grantor, p)).map((p) => p.name);
    const listed = [...d.permissions].sort().join(', ');
    const actual = [...gap].sort().join(', ');
    if (listed !== actual) {
      fail(
        `role_delegations: ${d.grantor} -> ${d.grantee}: permissions must be exactly the set `
        + `${d.grantee} holds and ${d.grantor} lacks.\n  declared: [${listed}]\n  actual:   [${actual}]`,
      );
    }
  }

  return { permissions, resources: reg.resources, retired, roles, delegations };
}
...
function emitGoDelegations(reg) {
  const entries = reg.delegations.map((d) => {
    const note = d.note ? goComment(d.note, '\t') + '\n' : '';
    return `${note}\t{
\t\tGrantorRole: ${JSON.stringify(d.grantor)},
\t\tGranteeRole: ${JSON.stringify(d.grantee)},
\t\tPermissions: []string{${d.permissions.map((n) => JSON.stringify(n)).join(', ')}},
\t},`;
  });
  return `// Code generated by scripts/generate-permissions.mjs from
// standards/permissions.yaml. DO NOT EDIT — edit the YAML and run
...
async function main() {
  ...
  const outputs = [
    ...
    [path.join(root, 'services', 'auth-service', 'internal', 'rbac', 'role_delegations_gen.go'),
      emitGoDelegations(reg)],
  ];
  ...
  if (checkOnly) {
    ...
    console.log(`permissions check OK (${reg.permissions.length} permissions, ${reg.roles.length} roles, ${reg.delegations.length} delegations, ${outputs.length} artefacts)`);
    return;
  }
...
function selfCheck(reg) {
  ...
  // A role-delegation exception must NOT have leaked into what the grantor
  // role HOLDS. Delegating a permission and holding it are different things,
  // and the difference is the entire point of the carve-out.
  //
  // This is checked against the EMITTED SQL filters rather than the reference
  // evaluator, because those filters are what the database and
  // assignRolePermissions actually run. Dropping billing.update from
  // tenant_admin's except_names — the wrong fix for this bug, and the one
  // that looks like it works — fails right here.
  const roleByName = new Map(reg.roles.map((r) => [r.name, r]));
  for (const d of reg.delegations) {
    for (const name of d.permissions) {
      const p = reg.permissions.find((x) => x.name === name);
      const row = { name: p.name, resource: p.resource, action: p.action, scope: p.scope };
      const grantor = roleByName.get(d.grantor);
      const grantee = roleByName.get(d.grantee);
      if (sqlEval(sqlGrantExpr(grantor), row) || !sqlEval(sqlRevokeExpr(grantor), row)) {
        fail(`self-check: delegation ${d.grantor} -> ${d.grantee} names ${name}, but ${d.grantor}'s grant filter HOLDS it — a delegation must never widen the grantor's own permissions`);
      }
      if (!sqlEval(sqlGrantExpr(grantee), row)) {
        fail(`self-check: delegation ${d.grantor} -> ${d.grantee} names ${name}, but ${d.grantee} does not hold it — the exception unblocks nothing`);
      }
    }
  }
}
