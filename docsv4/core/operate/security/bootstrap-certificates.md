...
Certificates are **not** generated automatically — `./scripts/bootstrap-env.sh`
only writes `.env`. Run the two scripts above yourself, once:
...
Skipping this step is fine — `cluster-sensor-service` detects the missing
bootstrap cert at startup, logs an informational message, and skips
auto-registration silently rather than crashing.
...
For a real production deployment, use the Kubernetes (Helm chart) path below
instead.
