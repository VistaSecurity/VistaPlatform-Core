...
- **CMDB pull** *(Enterprise)* — import server records from a connected CMDB (ServiceNow, Device42, SolarWinds, Oomnitza) as pending-approval assets
...
Generate immutable, content-hashed Cryptographic Bills of Materials (CBOM) in CycloneDX 1.7 format for supply-chain risk management, audit submissions, and regulatory filings. Each artifact is scoped to a named boundary. *Signing artifacts with HMAC, embedding compliance attestation layers, comparing two artifacts to show how posture changed over time, and SPDX/PDF output are the Enterprise evidence layer; Core generates the artifact and exports CycloneDX.* For convenience exports of current page views, each Inventory lens provides a one-click CSV export.

### Integrations
Store integration credentials with AES-256-GCM encryption.

*The following are Enterprise capabilities, absent from a Core build:* synchronizing inventory data with CMDBs such as ServiceNow, Device42, SolarWinds, and Oomnitza — **push** cryptographic metadata (optionally with a one-line crypto-posture summary appended to each CI's description) and **pull** server records back into the platform; forwarding audit events to an external SIEM; and federated identity (tenant OIDC/SAML, social sign-up, staff SSO). Core authenticates users with local accounts, invitations and RBAC.

## Architecture at a Glance

Vista Platform is a microservices platform built with Go 1.26 on the backend and React 19 (TypeScript, Vite, Tailwind CSS) on the frontend.
