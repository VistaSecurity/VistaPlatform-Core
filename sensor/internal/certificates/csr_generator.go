...
// GenerateKeyPair generates a new RSA keypair for the sensor
...
// GenerateCSR generates a Certificate Signing Request for a sensor
// sensorID should be the UUID string of the sensor ID
func (g *CSRGenerator) GenerateCSR(sensorID uuid.UUID, privateKey *rsa.PrivateKey) (string, error) {
	// Validate sensor ID
	if sensorID == uuid.Nil {
		return "", fmt.Errorf("sensor ID cannot be nil")
	}
